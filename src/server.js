...
async function handleRequest(req, res) {
  ...
  if (url.pathname === '/api/skills-hub/remove' && req.method === 'POST') {
    try {
      const body = await readJsonBody(req);
      sendJson(res, 200, { ok: true, ...(await removeSkill(body.name)) });
    } catch (error) {
      sendJson(res, 400, { ok: false, error: error.message || 'Skill removal failed.' });
    }
    return;
  }
